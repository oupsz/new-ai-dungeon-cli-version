...
class Config:
    def __init__(self):
        ...
        self.resume_target: str = None
        self.resume_last: bool = False
    ...
    def load_from_cli_args(self):
        ...
        if hasattr(parsed, "resume"):
            self.resume_target = parsed.resume
        if hasattr(parsed, "resume_last"):
            self.resume_last = parsed.resume_last
    ...
    def parse_cli_args():
        ...
        adventure_group = parser.add_mutually_exclusive_group()
        adventure_group.add_argument("--adventure", type=str, required=False,
                                     help="public multi-user adventure id to connect to")
        adventure_group.add_argument("--resume", type=str, required=False,
                                     help="resume a single-player adventure by shortId or adventureId")
        adventure_group.add_argument("--resume-last", action='store_const', const=True,
                                     help="resume the last locally saved single-player adventure")
        ...
        if parsed.name and not parsed.adventure:
            parser.error("--name is only supported together with --adventure")
